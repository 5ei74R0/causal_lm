...
class Args:
    ...
    seed: int = 5740
    num_workers: int = 8
    output_dir: str = "output/causal_lm"
...
def setup_model(tokenizer, n_layers) -> causal_lm.model.CausalTransformerNoPE:
    model = causal_lm.model.CausalTransformerNoPE(n_layers * 64, n_layers, n_layers, len(tokenizer))
...
def run():
    ...
    model = setup_model(tokenizer, args.n_layers)
    ...
    def evaluate(loader: DataLoader):
        ...
        for step, batch in enumerate(loader):
            logits, _, _ = model.forward(batch["input_ids"])
            losses.append(accelerator.gather(lm_loss(batch["input_ids"], logits)))
