...
class Args:
    ...
    seed: int = 5740
    num_workers: int = 8
    output_dir: str = "output/denoising_lm"
...
def setup_model(tokenizer, context_length, n_layers) -> GPT2LMHeadModel:
    ...
    class gpt2_nope_forward_fn:
        def __init__(self, gpt2_ref) -> None:
            self.ref: GPT2Model = gpt2_ref
            self.noisy_wte = deepcopy(self.ref.wte)  # use it as an initial noise generator
            self.noisy_wte.requires_grad_(False)
        ...
        def __call__(
            ...
            noise_embeds = self.noisy_wte(input_ids)  # generate noise
            position_embeds = self.ref.wpe(position_ids)
            hidden_states = inputs_embeds + position_embeds
            noise_embeds = noise_embeds + position_embeds
            ...
            for i, (block, layer_past) in enumerate(zip(self.ref.h, past_key_values)):
                ...
                if self.ref.gradient_checkpointing and self.ref.training:
                    outputs = self.ref._gradient_checkpointing_func(
                        ...
                        use_cache,
                        output_attentions,
                        noise_embeds if i == 0 else None,  # input noise only for the first layer
                    )
                else:
                    outputs = block(
                        hidden_states,
                        ...
                        noise_input=noise_embeds if i == 0 else None,  # input noise only for the first layer
                    )
    ...
    class layer_forward_fn:
        ...
        def __call__(
            ...
            noise_input: Optional[torch.FloatTensor] = None,
            ...
            # residual connection
            hidden_states = attn_output + (residual if noise_input is None else noise_input)
    ...
    model.transformer.forward = gpt2_nope_forward_fn(model.transformer)
